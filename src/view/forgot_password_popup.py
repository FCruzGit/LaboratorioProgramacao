...
class ForgotPasswordPopup(ctk.CTkToplevel):
    ...
    def _build_ui(self) -> None:
        ...
        ctk.CTkLabel(frame, text="Recuperar Senha", font=ctk.CTkFont(size=24, weight="bold"), text_color=TEXT_COLOR).grid(row=0, column=0, columnspan=2, sticky="w", padx=30, pady=(25, 15))

        campos = [("Usuário", False), ("Nome da Mãe", True), ("Nova Senha", True), ("Confirmar Senha", True)]
        ...
        ctk.CTkButton(btn_frame, text="Alterar Senha", font=ctk.CTkFont(size=14, weight="bold"), fg_color=BTN_PRIMARY, hover_color="#0D47A1", height=38, corner_radius=6, command=self._recuperar).pack(fill="x")
